...
//    }
    ...
    public static LocalDateTime convert() throws ParseException {
        ...
        Date dateTime = new SimpleDateFormat(DateFormatUtils.STANDARD_STAMP2).parse(dateStr);
    ...
    public static LocalDate convertDate(String dateStr) {
        try {
            Date date = new SimpleDateFormat(DateFormatUtils.STANDARD_DATE_DAY2).parse(dateStr);
    ...
    public static LocalDateTime getOffsetHours(LocalDateTime localDateTime, int offset, int symbol) {
        ...
        if(1 == symbol){
            offset = ~(offset - 1);
            cal.add(Calendar.HOUR, offset);
            return DateFormatUtils.asLocalDateTime(cal.getTime());
        }else {
            cal.add(Calendar.HOUR, offset);
            return DateFormatUtils.asLocalDateTime(cal.getTime());
        }
